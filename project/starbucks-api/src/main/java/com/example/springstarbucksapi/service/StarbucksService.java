...
public class StarbucksService {
    ...
    public void deleteAllOrders() {
        registersRepository.deleteAllInBatch();
    ...
    public StarbucksOrder newOrder(String regid, StarbucksOrder order) throws ResponseStatusException {
        ...
        StarbucksOrder active = getActiveOrder(regid);  //CHANGE make sql query to check if current register has a active drink
        if (active != null) {
    ...
    public void clearActiveOrder(String regid) {
        //CHANGE make sql query for active order
        StarbucksRegister register = registersRepository.findByRegister(regid);
    ...
    public StarbucksCard processOrder(String regid, String cardnum) throws ResponseStatusException {
        ...
        // Create new drink for active order that will
        // representing drinks being made/have been made
        StarbucksDrink drink = new StarbucksDrink();
        drink.setOrder(active);
        drink.setStatus("Drink Confirmed. In queue to be made");
        drinkRepository.save(drink);

        rabbit.convertAndSend(queue.getName(), Long.toString(drink.getId()));

        return card;
    }

    public StarbucksDrink setDrinkStatus(long drinkId, String status){
        List<StarbucksDrink> drinks = drinkRepository.findById(drinkId);
        if(!drinks.isEmpty()){
    ...
    public StarbucksDrink getDrink(long drinkId){
        List<StarbucksDrink> drinks = drinkRepository.findById(drinkId);
        if(!drinks.isEmpty()){
            return drinks.get(0);
        }
    ...
    public List<StarbucksDrink> getDrinks(){
